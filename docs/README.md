...
- [x] 자판기 보유 금액을 충전할 수 있다.
  - [x] 최초 자판기가 보유한 금액은 0원이여야하고, 각 동전의 개수는 0개여야 한다.
  - [x] 자판기 보유 금액은 `{금액}원` 형식으로 나타내야한다.
  - [x] 자판기 보유 금액 만큼의 동전을 무작위로 생성한다.
    - [x] 동전의 개수는 `{개수}개` 형식으로 나타내어야한다.
  - [x] 자판기 보유 금액을 누적하여 충전할 수 있다. 추가 충전 금액만큼의 동전이 무작위로 생성되어 기존 동전들에 더해져야한다.
  - [x] (예외 사항) 충전하는 금액은 10원으로 나누어 떨어져야한다.
- [x] 새로고침을 하더라도 가장 최근에 작업한 정보들을 불러올 수 있어야한다. (localStorage)
...
- [x] 최초 충전 금액은 0원이며, 반환된 각 동전의 개수는 0개여야한다.

- [x] 금액을 투입한다.
  - [x] 금액은 10원으로 나누어 떨어지는지 검증해야한다.
  - [x] 금액은 누적으로 투입할 수 있어야한다.
- [x] 투입한 금액을 보여준다.
  - [x] 자판기가 보유한 금액은 `{금액}원` 형식으로 나타내어야 한다.
- [x] 구매할 수 있는 상품 현황을 보여준다.
- [x] 물건을 구매한다.
- [ ] 잔돈을 반환 받는다. (잔돈 계산 모듈)
  - [ ] 현재 보유한 최소 개수의 동전으로 잔돈을 돌려줘야한다.
  - [ ] 지폐를 잔돈으로 반환하는 경우는 없어야한다.
  - [ ] 잔돈을 반환할 수 없는 경우 잔돈으로 반환할 수 있는 금액만 반환한다.
- [ ] 반환받은 잔돈(동전 종류와 개수)을 보여준다.
  - [ ] 동전의 개수를 나타내는 정보는 `{개수}개` 형식으로 나타내어야 한다.
